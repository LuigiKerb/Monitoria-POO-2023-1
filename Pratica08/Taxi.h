...
class Taxi : public VeiculoTrans, Carro{
    private:
        double tarifaInicial, valorKM;
        int distanciaKM;
    public:
        Taxi();
        Taxi(const double, const string, const int, const double, const double, const int, const int);
        ~Taxi();
...
Taxi::Taxi(const double p, const string m, const int n, const double t, const double v, const int d, const int) : VeiculoTrans(p,m,n), Carro(n){
